...
import {IKushPair} from "./interfaces/IKushPair.sol";
import {IKushRouter02} from "./interfaces/IKushRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by KushSwap teams.
 */
contract KushZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped WETH (WETH)
    IWETH public WETH;

    // KushRouter interface
    IKushRouter02 public kushRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from KushSwap)
    ...
    // Address KushRouter
    address private kushRouterAddress;

    // Address Wrapped WETH (WETH)
    ...
    // tokenToZap = 0x00 address if WETH
    event ZapIn(
    ...
    // token0ToZap = 0x00 address if WETH
    ...
    // tokenToReceive = 0x00 address if WETH
    event ZapOut(
        address indexed lpToken,
     ...
     * @param _kushRouter: address of the KushRouter
    ...
    constructor(
        ...
        kushRouterAddress = _kushRouter;
        kushRouter = IKushRouter02(_kushRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap WETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken: LP token address (e.g. PLANET/WETH)
     * @param _tokenAmountOutMin: minimum token amount (e.g. PLANET) to receive in the intermediary swap (e.g. WETH --> PLANET)
     */
    function zapInWETH(address _lpToken, uint256 _tokenAmountOutMin) external payable nonReentrant {
    ...
    /*
     * @notice Zap 1 token and WETH, rebalance them to 50-50, before adding them to LP
     * @param _token1ToZap: address of token1 to zap
     * @param _token1AmountIn: amount of token1 to zap
    ...
    /*
     * @notice Zap a LP token out to receive WETH
     * @param _lpToken: LP token address (e.g. PLANET/WETH)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in WETH/WETH) in the intermediary swap (e.g. PLANET --> WETH)
     */
    function zapOutWETH(
        address _lpToken,
     ...
     * @dev Use WETH for _tokenToZap (if WETH is the input)
    ...
    {
        address token0 = IKushPair(_lpToken).token0();
        address token1 = IKushPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IKushPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = kushRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = kushRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
     ...
     * @dev Use WETH for _token0ToZap (if WETH is the input)
    ...
    {
        require(
            _token0ToZap == IKushPair(_lpToken).token0() || _token0ToZap == IKushPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IKushPair(_lpToken).token0() || _token1ToZap == IKushPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IKushPair(_lpToken).token0()) {
    ...
    /*
     * @notice View the details for single zap
     * @dev Use WETH for _tokenToReceive (if WETH is the asset to be received)
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IKushPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IKushPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = kushRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = kushRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IKushPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "KushRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "KushRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        kushRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), kushRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(kushRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = kushRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = kushRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = kushRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = kushRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = kushRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
